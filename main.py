...
def run(start_date: int, end_date: int):
    ...
    for row in tqdm(results, ncols=80, miniters=len(results) // 5):  # 20% 단위로 표시
        ...
        if values["status"] != "CXL":  # 취소된 행사는 참조사항에 접근 불가함
            values["reference"] = scraper.get_reference_preview(
                row["FNC_NAME_ORG"], int(row["FNC_RSVN_NO"]), int(row["EVENT_NO"]), GW_REFERENCE_URL, SESSION_ID
            )
        ...
        if notion_properties:
            ...
            for prop_name in ["event_name", "start_time", "end_time", "place", "status", "manager", "reference"]:
                ...
                if previous_value != updated_value:
                    changed_properties.append({"item": prop_name, "previous_value": previous_value, "updated_value": updated_value})
